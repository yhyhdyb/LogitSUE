...
const node_id = ref(0)//传出去的index

watch(node_id, (node_id) => {
  store.commit('setSharedValue', node_id);
});
...
function recursion_del_layers(node_id:number) {
  ...
  while (now_pos < nodes_to_del.length) {
    let now_node_id = nodes_to_del[node_id]
...
function handleMenuRecursionDel() {
  ...
  // 在store中删除
  networkData.networkRecursionDelete(node_to_del)
