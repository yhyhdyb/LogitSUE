...
let maps =ref([] as L.Map[]) // 地图对象数组
const currentMapIndex = computed(() => store.getters.getSharedValue).value;
 // 当前地图索引
...
function map_show() {
  ...
  // 创建地图并设置中心点坐标和缩放级别
  map = L.map('map'+currentMapIndex).setView([43.5528027552607, -96.74700008207309], 12);
  map.setMinZoom(12);
  map.setMaxZoom(15);
  console.log("map: ", map);
  // 添加地图图层
  ...
  leafletImage(map , function(err: any, canvas: any) {
    ...
    console.log("img: ", img);
    var img1 = document.getElementById('thumbnail');
    if (img1 !== null) {
...
<template>
  <div class="main_window">
    ...
    <div class="map_box">
      <!-- <div v-for="(map, index) in maps" :key="index" :id="'map' + index" v-show="index === currentMapIndex"> -->
        <!-- 这里是你的地图 -->
