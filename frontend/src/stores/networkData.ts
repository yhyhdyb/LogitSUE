    ...
    historgram_data: (state) => {
      ...
      for (let i = 0; i < state.links_avg_time.length; i++) {
        if (
          ...
          // fftt
          if (state.links_avg_time[i] < bars_scope[0][0]) bars_scope[0][0] = state.links_avg_time[i]
          if (state.links_avg_flow[i] > bars_scope[0][1]) bars_scope[0][1] = state.links_avg_flow[i]
          ...
          // capacity
          if (state.links_avg_flow[i] < bars_scope[4][0])
            bars_scope[4][0] = state.links_avg_flow[i]
          if (state.links_avg_flow[i] > bars_scope[4][1])
            bars_scope[4][1] = state.links_avg_flow[i]
  ...
  actions: {
    ...
    update_links_global_info() {
      ...
      for (let i = 1; i < this.networksInfoArr.length; i++) {
        const tmp_rad = Math.abs(this.networksInfoArr[0].costSum / this.networksInfoArr[i].costSum - 1)
      ...
      for (let i = 0; i < this.networksInfoArr.length; i++) {
        ...
        for (let j = 0; j < this.networksInfoArr[i].links.length; j++) {
          ...
          // 判断global id是否超出范围
          // console.log('gid: ', now_link.globalId)

          // if (now_link.globalId >= tmp_links_infos2[0].length) {
          //   console.log("Issue: linksPos的长度与link的数量不匹配！");

          //   for (let k=tmp_links_infos2[0].length-1; k<now_link.globalId; k++) {
          //     for (let g=0; g<4; g++) {
          //       tmp_links_infos2[g].push([0,0])
          //       tmp_links_infos1[g].push(0)
          //     }
          //   }
          //   for (let k=this.links_if_sel.length-1; k<now_link.globalId; k++) {
          //     this.links_if_sel.push(false)
          //   }
          // }
          console.log('nid: ', i, ', gid: ', now_link.globalId)
